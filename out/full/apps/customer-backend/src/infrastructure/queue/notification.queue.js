...
try {
  ...
  notificationQueue.on("error", (error) => {
    if (error.code === "ECONNREFUSED") {
      Logger.warn(
        `⚠️ [Notification Queue] Redis connection refused. Queue will retry automatically.`
      );
    } else {
  ...
  Logger.warn(
    `⚠️ [Notification Queue] Failed to initialize queue (Redis may not be available):`,
    error.message
  );
...
export const addNotificationJob = async (type, data) => {
  ...
  if (!notificationQueue) {
    Logger.warn(`[Queue] ⚠️ Queue not available. Skipping job '${type}'`);
    return;
  }
  ...
  try {
    ...
    Logger.debug(
      `[Queue] 📥 Added job '${type}' for user ${data.userId || "unknown"}`
    );
  } catch (error) {
    // Use debounced logging for Redis errors
    if (error.message?.includes("max requests limit exceeded")) {
      Logger.debounced(
        "queue-redis-limit",
        60000,
        Logger.error,
        `[Queue] ❌ Redis quota exceeded. Notifications disabled.`
      );
    } else {
