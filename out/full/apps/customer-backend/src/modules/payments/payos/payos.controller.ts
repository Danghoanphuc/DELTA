...
import payos from "../../../infrastructure/payment/payos.client.js";
import { config } from "../../../config/env.config.js";
import { Logger } from "../../../shared/utils/index.js";
import { MasterOrder } from "../../../shared/models/master-order.model.js";
import { PAYMENT_STATUS, MASTER_ORDER_STATUS, SUB_ORDER_STATUS, IPrinterProfileWithUser } from "@printz/types";
...
export const createPaymentLink = async (
  ...
  try {
    ...
    Logger.info(`[PayOS] Created order ${masterOrder.orderNumber} with orderCode ${masterOrder.orderCode}`);

    // 5. Create PayOS payment link
    const YOUR_DOMAIN = config.clientUrl;
    const totalAmount = masterOrder.totalAmount;

    if (!masterOrder.orderCode) {
      throw new Error("Failed to generate order code for PayOS payment.");
    }
    ...
    Logger.info(`[PayOS] Payment link created for order ${masterOrder.orderNumber}`);

    // ✅ ZERO-EXIT PAYMENT: Include QR code and payment link ID
    res.status(200).json({
      checkoutUrl: paymentLinkData.checkoutUrl,
      qrCode: paymentLinkData.qrCode, // VietQR string for in-chat payment
      paymentLinkId: paymentLinkData.paymentLinkId, // For tracking
...
export const handlePayOSWebhook = async (
  ...
  try {
    ...
    Logger.info(`[PayOS Webhook] Received webhook for orderCode: ${webhookData.orderCode}`);
    ...
    if (
      ...
      if (order) {
        Logger.info(`✅ [PayOS Webhook] Payment successful for OrderCode: ${orderCode} - Order: ${order.orderNumber}`);
        ...
        if (order.paymentStatus !== PAYMENT_STATUS.PAID) {
          ...
          order.printerOrders?.forEach((printerOrder: any) => {
            if (printerOrder.printerStatus === SUB_ORDER_STATUS.PENDING) {
              printerOrder.printerStatus = SUB_ORDER_STATUS.PAID_WAITING_FOR_PRINTER;
            }
          });
          await order.save();
          
          Logger.info(`✅ [PayOS Webhook] Updated order ${order._id} status to PAID`);

          // Xóa giỏ hàng
          try {
            await cartService.clearCart(order.customerId);
            Logger.info(`✅ [PayOS Webhook] Cleared cart for user ${order.customerId}`);
          } catch (clearError) {
          ...
          try {
            ...
            Logger.info(`✅ [PayOS Webhook] Sent confirmation email to ${order.customerEmail}`);
          } catch (emailError) {
            Logger.error(`[PayOS Webhook] Failed to send confirmation email:`, emailError);
          }
          ...
          for (const printerOrder of order.printerOrders) {
            try {
              ...
              ).populate("user", "email displayName")) as unknown as IPrinterProfileWithUser;
              ...
              if (printerProfile?.user?.email) {
                ...
                Logger.info(`✅ [PayOS Webhook] Sent notification to printer: ${printerProfile.user.email}`);
              }
            } catch (emailError) {
              Logger.error(
          ...
          Logger.info(`ℹ️ [PayOS Webhook] Order ${order._id} was already paid.`);
        }
      } else {
        Logger.warn(`⚠️ [PayOS Webhook] Order not found with orderCode: ${orderCode}`);
      }
    } else {
      Logger.warn(`⚠️ [PayOS Webhook] Payment not successful. Code: ${webhookData.code}, Desc: ${webhookData.desc}`);
    }
