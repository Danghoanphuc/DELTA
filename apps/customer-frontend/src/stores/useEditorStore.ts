...
// TODO: Add proper editor types when editor feature is implemented
...
interface DecalItem extends EditorItem {
  // Add decal-specific properties
}

interface GroupItem extends EditorItem {
  type: "group";
  parentId?: string | null;
  name: string;
  isVisible: boolean;
    ...
    devtools(
      immer((set, get) => ({
        ...
        updateItem: (id, updates) =>
          set((state) => {
            const item = state.items.find((i: EditorItem) => i.id === id);
            if (item) {
        ...
        deleteItems: (ids) =>
          set((state) => {
            ...
            state.items = state.items.filter(
              (item: EditorItem) => !itemsToDelete.has(item.id)
            );
        ...
        reorderItems: (activeId, overId, containerId) =>
          set((state) => {
            const activeItem = state.items.find(
              (i: EditorItem) => i.id === activeId
            );
            const overItem = state.items.find(
              (i: EditorItem) => i.id === overId
            );
            ...
            const overIndex = state.items.findIndex(
              (i: EditorItem) => i.id === overId
            );
        ...
        selectItem: (itemId, isMultiSelect) =>
          set((state) => {
            if (!itemId) {
            ...
            const item = state.items.find((i: EditorItem) => i.id === itemId);
        ...
        groupSelectedItems: () =>
          set((state) => {
            ...
            const firstParentId = state.items.find(
              (i: EditorItem) => i.id === state.selectedItemIds[0]
            )?.parentId;
            const allHaveSameParent = state.selectedItemIds.every(
              (id: string) =>
                state.items.find((i: EditorItem) => i.id === id)?.parentId ===
                firstParentId
            );
            ...
            // Update children
            state.items.forEach((item: EditorItem) => {
        ...
        ungroupSelectedItem: () =>
          set((state) => {
            ...
            const selectedGroup = state.items.find(
              (i: EditorItem) => i.id === state.selectedItemIds[0]
            );
            ...
            // Remove group
            state.items = state.items.filter(
              (item: EditorItem) => item.id !== selectedGroup.id
            );
            state.selectedItemIds = [];
            state.isDirty = true;
...
export const selectFirstSelectedItem = (
  state: EditorStore
): EditorItem | null => {
  if (state.selectedItemIds.length !== 1) return null;
  return (
    state.items.find((item) => item.id === state.selectedItemIds[0]) || null
  );
};
