...
// Component con hiển thị Title (Giữ nguyên logic Scramble)
function TitleWithScramble({
  title,
  isActive,
  createdAt,
}: {
...
export function ChatHistorySidebar({
  ...
  // ✅ Dùng trực tiếp Store & API thay vì phụ thuộc ChatContext
  const {
  ...
  // ✅ FIX CRASH: Luôn đảm bảo safeList là mảng
  const filteredConversations = useMemo(() => {
    const safeList = conversations || []; // Fallback nếu props là null/undefined

    if (!searchQuery.trim()) return safeList;
    const query = searchQuery.toLowerCase();
    return safeList.filter((convo) =>
  ...
  const deleteChat = (e: React.MouseEvent, id: string, title: string) => {
    ...
    openDialog({
      ...
      onConfirm: async () => {
        ...
        if (currentConversationId === id) clearCurrentConversation();

        try {
          const success = await chatApi.deleteConversation(id);
          if (success) showStatus("success", "Đã xóa hội thoại");
          else {
            showStatus("error", "Không thể xóa");
            loadConversations({ type: "customer-bot" }); // Rollback
          }
        } catch {
                    ...
                    {editingId === convo._id ? (
                      ...
                      >
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && saveTitle(e, convo._id)
                          }
                          className="flex-1 min-w-0 py-0.5 px-1 text-sm border border-blue-400 rounded bg-white outline-none"
                        />
                        <button
                          onClick={(e) => saveTitle(e, convo._id)}
                  ...
                  {!editingId && (
                    ...
                    >
                      <button
                        ...
                        className={cn(
                          "p-1 rounded-md transition-opacity relative opacity-0 group-hover:opacity-100",
                          menuOpenId === convo._id && "opacity-100 bg-gray-200"
                        )}
                      >
                      ...
                      {menuOpenId === convo._id && (
                        <div
                          ref={menuRef}
                          className="absolute right-0 top-6 w-36 bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-50 animate-in fade-in zoom-in-95"
                        >
                          <button
                          ...
                          <button
                            onClick={(e) =>
                              deleteChat(e, convo._id, convo.title || "Chat")
                            }
