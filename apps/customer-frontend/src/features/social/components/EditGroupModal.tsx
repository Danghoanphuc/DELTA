...
export function EditGroupModal({
  ...
  const handleToggleRemove = (userId: string) => {
    if (userId === currentUser?._id)
      return toast.error("Không thể tự xóa mình ở đây");
  ...
  const availableFriends = useMemo(() => {
    ...
    const currentParticipantIds =
      conversation?.participants?.map((p: any) => p.userId._id || p.userId) ||
      [];
        ...
        <div className="max-h-[70vh] overflow-y-auto custom-scrollbar bg-white">
          ...
          {mode === "info" && (
            <div className="p-5 space-y-6">
              ...
              {/* Name Edit */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase mb-1.5 block tracking-wider">
                  Tên nhóm
                </label>
                <Input
                  value={groupName}
                ...
                <div className="space-y-1">
                  ...
                  {membersToAdd.length > 0 && (
                    <div className="px-3 py-2 bg-blue-50 rounded-lg text-xs text-blue-700 mb-2 font-medium flex items-center">
                      <UserPlus size={14} className="mr-2" /> Đang thêm{" "}
                      {membersToAdd.length} thành viên mới...
                    </div>
                  )}
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isRemoved && "opacity-50 bg-red-50 hover:bg-red-50"
                          )}
                        >
            ...
            <>
              ...
              {isAdmin && (
                ...
                >
                  ...
                  Lưu thay đổi{" "}
                  {membersToAdd.length > 0 && `(+${membersToAdd.length})`}
                </Button>
              )}
            </>
          ) : (
