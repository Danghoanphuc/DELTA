...
export function CreateGroupModal({
  ...
  const { setConversations, conversations, addConversation } =
    useSocialChatStore();
  ...
  const handleCreate = async () => {
    ...
    if (selectedMembers.length < 1)
      return toast.error("Chọn ít nhất 1 thành viên");
    ...
    try {
      ...
      if (res && res.conversation) {
        ...
        // Close modal FIRST before navigation to prevent UI glitch
        handleClose();
    ...
    // Don't reset isCreating on success - let modal close handle it
  };
  ...
  const selectedUsersInfo = friendsList
    .filter((f: any) => {
      const friend =
        f.friend ||
        (f.requester?._id
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    {filteredFriends.map((conn: any) => {
                      const friend =
                        conn.friend ||
                        (conn.requester?._id
                          ? conn.requester._id === currentUser?._id
                            ? conn.recipient
                            : conn.requester
                          : conn);
