// apps/admin-frontend/src/components/order-detail/AdminOrderThreadDialog.tsx
/**
 * Admin Order Thread Dialog - Chat dialog for admin to communicate with customer
 * Reuses AdminThreadDialog logic but for order context
 */
...
interface AdminOrderThreadDialogProps {
  orderId: string;
  orderNumber: string;
  open: boolean;
...
export function AdminOrderThreadDialog({
  orderId,
  orderNumber,
  open,
  onOpenChange,
}: AdminOrderThreadDialogProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await api.post(
        `/admin/order-threads/${thread._id}/messages`,
        {
      ...
      <DialogContent className="max-w-2xl h-[600px] flex flex-col p-0">
        ...
        <div className="border-t px-6 py-4">
          <div className="flex gap-3">
            <textarea
              placeholder="Nhập tin nhắn với khách hàng..."
              value={messageInput}
              onChange={(e) => setMessageInput(e.target.value)}
              onKeyDown={handleKeyDown}
              className="flex-1 min-h-[60px] max-h-[120px] resize-none border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
