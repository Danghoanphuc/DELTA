// apps/customer-backend/src/scripts/seed-swag-orders.js
/**
 * Seed SwagOrders for Organization Dashboard
 *
 * This creates SwagOrders that show up at:
 * /organization/dashboard?tab=swag-orders
 */
...
async function seedSwagOrders() {
  try {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("🚀 SEEDING SWAG ORDERS FOR ORGANIZATION");
    ...
    if (!user) {
      log.error(`User ${customerEmail} not found!`);
    ...
    console.log(
      `   Price: ${(
        swagPack.pricing?.unitPrice || 620000
      ).toLocaleString()} VND`
    );
    ...
    for (const order of createdOrders) {
      ...
      console.log(
        ...
        } | Total: ${order.pricing?.total?.toLocaleString()} VND`
      );
    }

    console.log("\n" + "=".repeat(60));
    console.log("🎉 SWAG ORDERS SEEDED SUCCESSFULLY!");
    console.log("=".repeat(60));

    console.log(`\n📝 NOW GO TO:`);
    console.log(
      `   http://localhost:5173/organization/dashboard?tab=swag-orders`
    );
    console.log(`   Login as: ${customerEmail}`);
    console.log("\n");
  } catch (error) {
