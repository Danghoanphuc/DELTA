...
const PrinterOrderSchema = new mongoose.Schema({
  printerProfileId: { type: mongoose.Schema.Types.ObjectId, ref: "PrinterProfile", required: true },
  ...
  printerStatus: { type: String, enum: Object.values(SUB_ORDER_STATUS), default: SUB_ORDER_STATUS.PENDING },
  artworkStatus: { type: String, enum: ["pending_upload", "pending_approval", "approved", "rejected"], default: "pending_upload" },
  proofFiles: [{
      ...
      status: { type: String, enum: ["current", "superseded", "rejected"], default: "current" },
  }],
  rejectionHistory: [{
...
const MasterOrderSchema = new mongoose.Schema({
    ...
    paidAt: { type: Date },
    masterStatus: { type: String, enum: Object.values(MASTER_ORDER_STATUS), default: MASTER_ORDER_STATUS.PENDING_PAYMENT, index: true },
