...
import { Canvas, ActiveSelection, util } from "fabric"; // <-- Import util
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (modKey && e.key.toLowerCase() === "c") {
        ...
        if (activeObject) {
          // SỬA LỖI: Phải clone đối tượng, không gán trực tiếp
      ...
      if (modKey && e.key.toLowerCase() === "v") {
        ...
        if (clipboardRef.current && canvas.current) {
          // SỬA LỖI: Phải clone từ clipboard, không thêm trực tiếp
